...
use super::keyboard::{
    KEYMAP, KEYMAP_CTRL, KEYMAP_META, KEYMAP_META_CTRL, KEYMAP_META_SHIFT, KEYMAP_SHIFT,
};
use crate::{
    arch::{
        ints::{ExceptionStackFrame, IDTEntry, IDTFlags, SyscallRegisters, IDT},
        tasks::{context_switch, idle_until_switch},
    },
    console::get_console,
    tasks::{num_tasks, CURRENT_TERMINATED, IN_TASK},
};
use x86::io::{inb, outb};
...
unsafe extern "x86-interrupt" fn keyboard_handler(_frame: ExceptionStackFrame) {
    ...
    if input == 0xe0 {
        ...
        if let Some(console) = get_console() {
            let keysym = if console.get_alt() && console.get_ctrl() {
