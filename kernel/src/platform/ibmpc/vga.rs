...
impl RawTextConsole for VGAConsole {
    fn write_char(&mut self, x: u16, y: u16, color: ColorCode, c: char) {
        let character = if c as u32 > 0x100 { b'?' } else { c as u8 };
        self.buffer[y as usize * self.width + x as usize] = (((color.background as u16) & 0xf)
            << 12)
    ...
    fn clear(&mut self, x0: u16, y0: u16, x1: u16, y1: u16, color: ColorCode) {
        let color2 =
            (((color.background as u16) & 0xf) << 12) | (((color.foreground as u16) & 0xf) << 8);
        for y in y0..=y1 {
            for x in x0..=x1 {
            ...
            Ordering::Less => {
                ...
                for y in (y0..y0 + height).rev() {
                    ...
                    for i in 0..self.width {
                        self.buffer[(y + diff) as usize * self.width + i] =
                            self.buffer[y as usize * self.width + i];
                    }
                }
            }
            Ordering::Greater => {
