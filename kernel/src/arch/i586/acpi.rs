...
fn read_header(phys_addr: u64) -> Option<ACPIHeader> {
    ...
    unsafe { map_memory(&mut get_page_dir(None), &[page, page + 1], |s| *(&s[offset] as *const u8 as *const ACPIHeader)).ok() }
}
...
fn read_data(phys_addr: u64, len: u32) -> Option<Vec<u8>> {
    ...
    unsafe { map_memory(&mut get_page_dir(None), &addresses, |s| s[offset..offset + len].to_vec()).ok() }
}
...
fn read_sdt<S: SDTPointer + Clone>(phys_addr: u64) -> Option<SDT<S>> {
    ...
    unsafe { map_memory(&mut get_page_dir(None), &[page, page + 1], |s| SDT::from_raw_pointer(&s[offset] as *const u8)).ok() }
}
...
fn find_rsdp() -> Option<u64> {
    ...
    for page in (0x000e0000..0x00100000).step_by(PAGE_SIZE) {
        unsafe {
            if let Some(addr) = map_memory(&mut get_page_dir(None), &[page], |s| {
...
fn read_rsdp(phys_addr: u64) -> Option<RSDP> {
    ...
    unsafe {
        map_memory(&mut get_page_dir(None), &[page, page + 1], |s| {
