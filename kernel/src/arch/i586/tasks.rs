...
use super::{
    ...
    paging::{alloc_pages_at, free_page_phys, PageDirectory, PageTableFlags, PAGE_DIR},
    syscalls::{read_handler, write_handler},
    LINKED_BASE, PAGE_SIZE,
};
use crate::{
    tasks::{
        add_page_reference, add_task, get_current_task, get_current_task_mut, get_page_references,
        get_task, get_task_mut, remove_page_reference, remove_task, switch_tasks, BlockKind, Task,
        CURRENT_TERMINATED, IN_TASK,
    },
    types::errno::Errno,
};
use alloc::{
...
impl TaskState {
        ...
        for i in start..end {
            if dir.tables[i].is_null() {
                ...
                for addr in ((i << 22)..((i + 1) << 22)).step_by(PAGE_SIZE) {
                    let orig_page = unsafe {
                        &mut *dir
                            .get_page(addr as u32, false)
                            .expect("couldn't get page table")
                    };
    ...
    fn map_task_in(&mut self, addr: u64, len: u64, is_writable: bool) -> Result<MappedMem, Errno> {
        ...
        let buf_len = (end - start)
            .try_into()
            .map_err(|_| Errno::NotEnoughSpace)?;
        ...
        let mut existing_phys: Vec<u64> =
            Vec::with_capacity(((end - start) / PAGE_SIZE_U64) as usize);
        ...
        for i in (start..end).step_by(PAGE_SIZE) {
            ...
            let phys_addr =
                match self.virt_to_phys(i.try_into().map_err(|_| Errno::NotEnoughSpace)?) {
                    Some(phys) => phys,
                    None => self.alloc_page(
                        i.try_into().map_err(|_| Errno::NotEnoughSpace)?,
                        false,
                        is_writable,
                        false,
                    ) as u32,
            ...
            assert!(
                !existing_phys.contains(&(phys_addr as u64)),
                "kernel trampling on process memory"
            );
        ...
        let data = unsafe {
            core::slice::from_raw_parts_mut(
                (ptr as usize + offset as usize) as *mut u8,
                len.try_into().map_err(|_| Errno::NotEnoughSpace)?,
            )
        };
    ...
    pub fn find_hole(&mut self, start: usize, size: usize) -> Option<usize> {
        ...
        for i in 0..(LINKED_BASE >> 22) {
            if self.pages.tables[i].is_null() {
                ...
                } else if if let Some(start) = hole_start {
                    addr - start <= size
                } else {
                    false
                } {
                ...
                for addr in ((i << 22)..((i + 1) << 22)).step_by(PAGE_SIZE) {
                    let orig_page = unsafe {
                        &mut *self.pages.get_page(addr as u32, false).expect("couldn't get page table")
                    };

                    if orig_page.is_unused() {
                        if if let Some(start) = hole_start {
                            addr - start <= size
                        } else {
                            false
                        } {
...
pub fn fork_task(id: usize) -> Result<&'static mut Task, &'static str> {
    let current = if let Some(task) = get_task_mut(id) {
        task
    } else {
