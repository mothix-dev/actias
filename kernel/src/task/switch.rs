use super::{cpu::ThreadID, get_cpus, get_process, queue::TaskQueueEntry};
use crate::{
...
fn _context_switch(timer_num: usize, cpu: Option<ThreadID>, regs: &mut Registers, manual: bool, was_in_kernel: bool, mode: ContextSwitchMode) {
    ...
    if !was_in_kernel && (queue.len() > 0 || mode != ContextSwitchMode::Normal) {
            ...
            match mode {
                ...
                ContextSwitchMode::Remove => {
                    if let Some(id) = remove_id {
                        if let Some(mut process) = get_process(id.process) {
                            process.remove_thread(id.thread);
                        } else {
                            error!("couldn't get process {id} for removing thread");
                        }
                    } else {
...
pub fn context_switch_timer(timer_num: usize, cpu: Option<ThreadID>, regs: &mut Registers, was_in_kernel: bool) {
    _context_switch(timer_num, cpu, regs, false, was_in_kernel, ContextSwitchMode::Normal);
}
...
pub fn manual_context_switch(timer_num: usize, cpu: Option<ThreadID>, regs: &mut Registers, mode: ContextSwitchMode) {
    _context_switch(timer_num, cpu, regs, true, false, mode);
}
