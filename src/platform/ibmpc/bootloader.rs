    ...
    pub fn copy(&self) -> MultibootInfoCopy {
        ...
        let modules_copy =
            if let Some(modules) = self.get_modules() {
                ...
                if len > 0 {
                    let new = unsafe { slice::from_raw_parts_mut(kmalloc::<MultibootModuleCopy>(len * size_of::<MultibootModuleCopy>(), false).pointer, len) };
...
impl MultibootModule {
    ...
    pub fn get_contents(&self) -> &[u8] {
        unsafe {
            slice::from_raw_parts(self.start as usize as *const u8, self.end as usize - self.start as usize)
        }
    }
...
pub fn reserve_pages(set: &mut BitSet) {
    ...
    fn set_region(set: &mut BitSet, start: u64, end: u64, state: bool) {
        ...
        let start_page =
            if start % PAGE_SIZE_U64 != 0 { // if our base address doesn't align to a page boundary, round up to the nearest page boundary
    ...
    if let Some(iter) = (&mut iter).as_mut() {
        ...
        for region in iter {
            if region.kind == MappingKind::Available {
                ...
                set_region(set, region.base_addr, region.base_addr + region.length, false);
            }
        }
    } else {
    ...
    unsafe { MULTIBOOT_INFO = Some(info.copy()); }
}
