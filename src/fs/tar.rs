...
impl<N: ArrayLength<u8>> From<&TarNumber<N>> for usize {
    fn from(num: &TarNumber<N>) -> Self {
        Self::from_str_radix(num.to_str(), 8).expect("couldn't parse numeric value")
    }
}
...
impl Directory for TarDirectory {
    ...
    fn set_owner(&mut self, _owner: UserID) -> Result<(), Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }
    ...
    fn set_group(&mut self, _group: GroupID) -> Result<(), Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }
    ...
    fn create_file(&mut self, _name: &str, _permissions: Permissions) -> Result<(), Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }

    fn create_directory(&mut self, _name: &str, _permissions: Permissions) -> Result<(), Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }

    fn create_link(&mut self, _name: &str, _target: &str, _permissions: Permissions) -> Result<(), Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }
...
impl File for TarFile {
    ...
    fn write_at(&mut self, _bytes: &[u8], _offset: u64) -> Result<usize, Errno> {
        Err(Errno::ReadOnlyFileSystem)
    }
    ...
    fn read_at(&self, bytes: &mut [u8], offset: u64) -> Result<usize, Errno> {
        if offset >= self.contents.len() as u64 {
            ...
            let offset: usize = offset.try_into().map_err(|_| Errno::ValueOverflow)?;
            let size = if offset + bytes.len() > self.contents.len() { self.contents.len() - offset } else { bytes.len() };
            bytes[offset..offset + size].copy_from_slice(&self.contents[..size]);
            Ok(size)
        }
    }

    fn can_read_at(&self, space: usize, offset: u64) -> bool {
        (offset + space as u64) >= self.contents.len() as u64
    }
...
fn mkdir(root: &mut Box<dyn Directory>, path: &str, permissions: Permissions, owner: UserID, group: GroupID) {
    let elements = path.split('/').collect::<Vec<_>>();

    fn make_dir(root: &mut Box<dyn Directory>, elements: &Vec<&str>, extent: usize, permissions: Permissions, owner: UserID, group: GroupID) {
