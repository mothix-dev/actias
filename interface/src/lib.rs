...
use common::types::{Errno, MmapAccess, MmapFlags, ProcessID, Result, Syscalls};
use core::arch::asm;
...
pub fn mmap(id: u32, addr_hint: *mut u8, length: usize, access: MmapAccess, flags: MmapFlags) -> Result<*mut u8> {
    unsafe {
        syscall_4_args(
            Syscalls::Mmap,
            id,
            ...
            ((u8::from(access) as u32) << 8) | (u8::from(flags) as u32),
        )
...
pub fn share_memory(addr: *const u8, length: usize, access: MmapAccess) -> Result<u32> {
    unsafe {
        syscall_3_args(
            Syscalls::ShareMemory,
            (addr as usize).try_into().map_err(|_| Errno::ValueOverflow)?,
            length.try_into().map_err(|_| Errno::ValueOverflow)?,
            u8::from(access) as u32,
        )
    }
}
...
pub fn set_message_handler(message: u32, priority: u8, handler: extern "fastcall" fn(u32)) -> Result<()> {
    unsafe {
        syscall_4_args(Syscalls::MessageHandler, message, priority as u32, handler as u32, 0)?;
    }

    Ok(())
}

pub fn set_message_handler_with_data(message: u32, priority: u8, handler: extern "fastcall" fn(u32, u32, u32)) -> Result<()> {
    unsafe {
        syscall_4_args(Syscalls::MessageHandler, message, priority as u32, handler as u32, 1)?;
    }
